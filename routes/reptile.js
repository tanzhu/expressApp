  ...
  fetch.on({
    ...
    itemList: function (itemList) {
      ...
      for (var i = 0; i < itemList.length; i++) {
        var item = itemList[i];
        var category = { "category_name": item.name, "category_url": item.url };
        collection.update({ "category_name": item.name }, { $set: { "category_url": item.url } }, { "options": { upsert: true} }, function (err, doc) {
          if (err)
            console.error(err);
          else {
        ...
        // collection.find({"category_name":item.name},function(e, docs, item){
        //   if(docs.length > 0){
        //     var category = docs[0];
        //     if(category.url != item.url){
        //       category.category_url = item.url;
        //       collection.update({"category_name":item.name},category);
        //     }
        //   }
        //   else {
        //     var category = {"category_name":item.name, "category_url":item.url};
